...
def create(director):
    ...
    # get existing director for check purposes
    existing_director = (
        Director.query
...
def read_one(id):
    ...
    :return:            director matching the id
    """

    # get the director based on id argument
    director = (
...
def read_by_name(name):
    """
    This function responds to a request for /api/directors/name/{name}
    with one matching director from directors
    ...
    # Otherwise, nope, didn't find that director
        ...
        abort(404, f"Director's name not found for: {name}")